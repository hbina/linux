...
It tracks which phases of the study plan have been completed, which are in progress, and what was covered in the last session. Use it to:
...
Gemini CLI **must not** automatically mark a phase or sub-phase as complete in `docs/PROGRESS.md`. A phase is only considered complete when the user explicitly states: "I have completed Phase X.Y" or similar. Until then, treat the phase as "In Progress" or "Not Started".
...
While Gemini CLI has built-in tools like `grep_search` and `codebase_investigator`, the following shell commands are useful for deep kernel exploration:

```bash
# Search kernel symbols (if grep_search is too narrow)
