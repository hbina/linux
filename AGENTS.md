...
This file provides foundational guidance to coding agents when working with code in this repository.
...
- Agents **must not** automatically mark a phase or sub-phase as complete in `docs/PROGRESS.md`.
- Only tick off items (`[ ]` -> `[x]`) or update phase status when the user
  explicitly states they have completed or understood that item or phase.
